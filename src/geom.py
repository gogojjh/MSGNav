...
def get_proper_snapshot_observation_point(
    ...
    # obs_dist: the distance between the observation point and the center in voxel space

    snapshot_center = np.mean(obj_centers, axis=0)
    if len(obj_centers) == 2:
        # if there are two objects, then the long axis is the line bewteen the two objects
    ...
    for idx in indices_rank:
        ...
        # ensure there is no occupied point between the observation point and the center
        direction = snapshot_center - potential_obs_point
        direction = direction / np.linalg.norm(direction)
        # adjust the center: usually there are surrounding occupied regions around it
        ...
        # then check whether there are occupied points between the observation point and the adjusted center
        if check_distance(
            occupied_map=np.logical_not(unoccupied_map),
            pos=potential_obs_point,
            direction=direction / np.linalg.norm(direction),
    ...
    # if no proper observation point is found, return the original observation position
    logging.error(
