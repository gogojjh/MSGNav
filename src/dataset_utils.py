...
def prepare_sg3d_navigation_goals(
    scene_name,
    episode,
    all_navigation_goals,
):
    ...
    for goal in episode["tasks"]:
        ...
        goal_category, goal_type, goal_object_id, sub_sentence = goal
        if '. 1.' in sub_sentence:
            main_question, step_question = sub_sentence.split('. 1. ')
            step_question = step_question
        else:
            step_question = sub_sentence.split('. ')[1]
        ...
        goal_inst = [
            x for x in dset_same_cat_goals[0] if x["object_id"] == goal_object_id
        ]
        assert (
            len(goal_inst) == 1
        ), f"more than 1 goal "
        goal_inst.append((main_question, step_question))
        all_subtask_goals.append(goal_inst)
    return all_subtask_goal_types, all_subtask_goals