...
from src.conceptgraph.utils.vis import OnlineObjectRenderer
from src.conceptgraph.utils.ious import compute_2d_box_contained_batch
from src.conceptgraph.utils.general_utils import to_tensor

from src.conceptgraph.slam.slam_classes import MapObjectList, DetectionList
...
def compute_match_batch(
    cfg, spatial_sim: torch.Tensor, visual_sim: torch.Tensor
) -> torch.Tensor:
    """
    ...
    if cfg.match_method == "sim_sum":
        sims = (1 + cfg.phys_bias) * spatial_sim + (
            1 - cfg.phys_bias
        ) * visual_sim  # (M, N)
...
@hydra.main(
    version_base=None, config_path="../configs/slam_pipeline", config_name="base"
)
def main(cfg: DictConfig):
    ...
    for idx in trange(len(dataset)):
        ...
        detections_path = (
            color_path.parent.parent / cfg.detection_folder_name / color_path.name
        )
            ...
            for detected_object in bg_detection_list:
                ...
                if bg_objects[class_name] is None:
                    ...
                    bg_objects[class_name] = merge_obj2_into_obj1(
                        cfg, matched_obj, matched_det, run_dbscan=False
                    )
        ...
        if cfg.save_objects_all_frames:
            ...
            if not cfg.skip_bg:
                bg_objects_to_save = MapObjectList(
                    [_ for _ in bg_objects.values() if _ is not None]
                )
        ...
        if cfg.vis_render:
            objects_vis = MapObjectList(
                [
                    copy.deepcopy(_)
                    for _ in objects
                    if _["num_detections"] >= cfg.obj_min_detections
                ]
            )
