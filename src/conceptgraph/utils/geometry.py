...
def transform_points_batch(poses: torch.Tensor, points: torch.Tensor) -> torch.Tensor:
    ...
    points_homogeneous = torch.cat(
        (points, torch.ones(N, 1).to(points.device)), dim=-1
    )  # (N, 4)
    ...
    points_transformed = torch.bmm(
        poses, points_homogeneous.transpose(1, 2)
    )  # (M, 4, N)

    # Convert back to cartesian coordinates by dividing by the last coordinate
    points_transformed = (
        points_transformed[:, :3, :] / points_transformed[:, 3:4, :]
    )  # (M, 3, N)
...
def project_points_camera2plane(points: torch.Tensor, K: torch.Tensor) -> torch.Tensor:
    ...
    points_coord = points_proj[:2, :] / points_proj[2, :].unsqueeze(
        0
    )  # divide x and y by z
...
def project_points_world2plane(
    points: torch.Tensor, poses: torch.Tensor, K: torch.Tensor
) -> torch.Tensor:
    ...
    points_coord, points_depth = project_points_camera2plane(
        points_camera, K
    )  # (M*N, 2), (M, N)
...
def check_proj_points(
    ...
    proj_xyz = torch.cat([proj_xy, proj_z.unsqueeze(-1)], dim=-1)  # (N, 3)

    proj_xy_within = torch.stack(
        [
            0 <= proj_xy[:, 0],
            proj_xy[:, 0] < depth_tensor.shape[1],
            0 <= proj_xy[:, 1],
            proj_xy[:, 1] < depth_tensor.shape[0],
        ],
        dim=-1,
    )  # (N, 2)
    proj_xy_within = torch.all(proj_xy_within, dim=-1)  # (N, )
    proj_z_within = proj_z > 0
    proj_within = torch.logical_and(proj_xy_within, proj_z_within)  # (N, )
    ...
    if proj_within.any():
        depth_within = depth_tensor[
            proj_xy[proj_within][:, 1].long(), proj_xy[proj_within][:, 0].long()
        ]  # (N, )
        proj_z_within = proj_z[proj_within]  # (N, )

        proj_front[proj_within] = proj_z_within < depth_within - depth_margin  # (N, )
        proj_align[proj_within] = (
            torch.abs(proj_z_within - depth_within) <= depth_margin
        )  # (N, )
