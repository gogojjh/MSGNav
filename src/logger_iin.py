...
class Logger:
    ...
    def init_subtask(
        ...
        view_pos_dict = goals[0]["view_points"][0]["agent_state"]
        obs, _ = scene.get_observation(
            pts=view_pos_dict["position"], rotation=view_pos_dict["rotation"]
        )
        plt.imsave(
            os.path.join(self.output_dir, episode_id, "image_goal.png"),
            obs["color_sensor"],
        )
        subtask_metadata["image"] = f"{self.output_dir}/{episode_id}/image_goal.png"
